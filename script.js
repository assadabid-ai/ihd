...
(function () {
  ...
  // The deck is a 17-slide main narrative followed by numbered backup slides.
  // Appendix slides are excluded from the counter and progress bar so the
  // audience sees "n / 17", not a total inflated by material we may not show.
  function isAppendix(s) { return s.classList.contains("appendix"); }
  var mainTotal     = slides.filter(function (s) { return !isAppendix(s); }).length;
  var appendixTotal = total - mainTotal;
  var firstAppendix = mainTotal;   // appendix slides are all trailing
  ...
  function render() {
    ...
    if (isAppendix(slides[current])) {
      counter.textContent = "A" + (current - firstAppendix + 1) + " / A" + appendixTotal;
      progress.style.width = "100%";
    } else {
      counter.textContent = (current + 1) + " / " + mainTotal;
      progress.style.width = ((current + 1) / mainTotal * 100) + "%";
    }
  ...
  document.addEventListener("keydown", function (e) {
    ...
    switch (e.key) {
      ...
      // "A" jumps to the first backup slide and back — for Q&A, where the
      // useful move is to reach the appendix without arrowing through it.
      case "a": case "A":
        if (appendixTotal) {
          go(current >= firstAppendix ? mainTotal - 1 : firstAppendix);
          e.preventDefault();
        }
        break;
      case "f": case "F":
